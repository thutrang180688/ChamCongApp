...
app.get('/api/data', async (req, res) => {
  ...
  try {
    ...
    const attRes = await sheets.spreadsheets.values.get({ spreadsheetId, range: 'Attendance!A:G' });
    ...
    const setRes = await sheets.spreadsheets.values.get({ spreadsheetId, range: 'Settings!A:G' });
...
app.post('/api/data', async (req, res) => {
  ...
  try {
    ...
    if (setIndex === -1) {
      await sheets.spreadsheets.values.append({ spreadsheetId, range: 'Settings!A:G', valueInputOption: 'RAW', requestBody: { values: [settingsRow] } });
    } else {
      await sheets.spreadsheets.values.update({ spreadsheetId, range: `Settings!A${setIndex + 1}:G${setIndex + 1}`, valueInputOption: 'RAW', requestBody: { values: [settingsRow] } });
    }
    ...
    for (const dateId in attendance) {
      ...
      const rowData = [email, dateId, record.type, record.note || '', record.chromeActiveTime || 0, record.isAutoClocked ? 'TRUE' : 'FALSE', record.isManual ? 'TRUE' : 'FALSE'];

      if (rowIndex !== -1) {
        updates.push({ range: `Attendance!A${rowIndex + 1}:G${rowIndex + 1}`, values: [rowData] });
      } else {
    ...
    if (appends.length > 0) {
      await sheets.spreadsheets.values.append({ spreadsheetId, range: 'Attendance!A:G', valueInputOption: 'RAW', requestBody: { values: appends } });
    }
...
app.get('/api/admin/users', async (req, res) => {
  if (req.session?.user?.email !== SUPER_ADMIN_EMAIL) return res.status(403).json({ error: 'Forbidden' });
  if (!process.env.GOOGLE_SHEET_ID) return res.status(400).json({ error: 'Sheet ID not configured' });
  ...
  try {
    ...
    const users = rows.slice(1).map(r => ({ email: r[0], name: r[1], lastLogin: r[2] }));
    res.json({ users });
  } catch (error) {
